...
export async function POST(req: Request) {
  ...
  try {
    const {
      ...
      startTime,
      endTime,
      secondsPerQuestion = { easy: 20, moderate: 35, difficult: 60, extreme: 90 },
      questionIds,
      course,
      subject,
      chapter,
      ...
      endTime: string;   // ISO
      secondsPerQuestion?: { easy: number; moderate: number; difficult: number; extreme: number };
      questionIds: string[];
      course?: string;
      subject?: string;
      chapter?: string;
    ...
    const parsedSecondsPerQuestion = {
      easy: Number(secondsPerQuestion.easy),
      moderate: Number(secondsPerQuestion.moderate),
      difficult: Number(secondsPerQuestion.difficult),
      extreme: Number(secondsPerQuestion.extreme),
    };

    if (
      Object.values(parsedSecondsPerQuestion).some(
        (value) => Number.isNaN(value) || value <= 0
      )
    ) {
      return NextResponse.json(
        { success: false, error: "Invalid secondsPerQuestion values" },
        { status: 400 }
      );
    }
    ...
    const created = await TeacherExamPaper.create({
      ...
      startTime: st,
      endTime: et,
      secondsPerQuestion: parsedSecondsPerQuestion,
