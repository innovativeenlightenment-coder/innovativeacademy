...
export async function POST(req: Request) {
  ...
  try {
    ...
    */
    ...
    /* ================= DURATION ================= */
    const defaultSecondsPerQuestion: SecondsPerQuestion = {
      easy: 20,
      moderate: 35,
      difficult: 60,
      extreme: 90,
    ...
    const rawSecondsPerQuestion = exam.secondsPerQuestion as any;
    const secondsPerQuestion: SecondsPerQuestion = {
      easy:
        rawSecondsPerQuestion && typeof rawSecondsPerQuestion === "object" && !Number.isNaN(Number(rawSecondsPerQuestion.easy))
          ? Number(rawSecondsPerQuestion.easy)
          : defaultSecondsPerQuestion.easy,
      moderate:
        rawSecondsPerQuestion && typeof rawSecondsPerQuestion === "object" && !Number.isNaN(Number(rawSecondsPerQuestion.moderate))
          ? Number(rawSecondsPerQuestion.moderate)
          : defaultSecondsPerQuestion.moderate,
      difficult:
        rawSecondsPerQuestion && typeof rawSecondsPerQuestion === "object" && !Number.isNaN(Number(rawSecondsPerQuestion.difficult))
          ? Number(rawSecondsPerQuestion.difficult)
          : defaultSecondsPerQuestion.difficult,
      extreme:
        rawSecondsPerQuestion && typeof rawSecondsPerQuestion === "object" && !Number.isNaN(Number(rawSecondsPerQuestion.extreme))
          ? Number(rawSecondsPerQuestion.extreme)
          : defaultSecondsPerQuestion.extreme,
    };

    const durationSec = questions.reduce((sum, question) => {
      const difficulty = String(question.difficulty || "easy").toLowerCase();
      return sum + (secondsPerQuestion[difficulty as keyof SecondsPerQuestion] ?? secondsPerQuestion.easy);
    }, 0);
