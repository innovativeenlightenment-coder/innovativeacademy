...
export async function PATCH(req: Request) {
  ...
  try {
    ...
    if (body.secondsPerQuestion !== undefined) {
      if (
        typeof body.secondsPerQuestion !== "object" ||
        body.secondsPerQuestion === null ||
        Array.isArray(body.secondsPerQuestion)
      ) {
        return NextResponse.json(
          { success: false, error: "Invalid secondsPerQuestion" },
          { status: 400 }
        );
      }

      const spqEntries = Object.entries(body.secondsPerQuestion);
      if (spqEntries.length === 0) {
        return NextResponse.json(
          { success: false, error: "secondsPerQuestion must contain at least one level" },
          { status: 400 }
        );
      }

      for (const [level, value] of spqEntries) {
        if (typeof value !== "number" || Number.isNaN(value) || value < 0) {
          return NextResponse.json(
            { success: false, error: `Invalid secondsPerQuestion for ${level}` },
            { status: 400 }
          );
        }
      }

      update.secondsPerQuestion = body.secondsPerQuestion;
    }
