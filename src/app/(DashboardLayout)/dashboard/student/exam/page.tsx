...
const normalizeSecondsPerQuestion = (
  secondsPerQuestion?: number | SecondsPerQuestion
): SecondsPerQuestion => {
  if (!secondsPerQuestion || typeof secondsPerQuestion === "number") {
    return { easy: 20, moderate: 35, difficult: 60, extreme: 90 };
  }

  return {
    easy: Number(secondsPerQuestion.easy ?? 20),
    moderate: Number(secondsPerQuestion.moderate ?? 35),
    difficult: Number(secondsPerQuestion.difficult ?? 60),
    extreme: Number(secondsPerQuestion.extreme ?? 90),
  };
};
  ...
  const startExam = async (examId: string) => {
    ...
    try {
      ...
      // Count difficulty levels
          const levelCounts = questions.reduce((acc: Record<string, number>, q: Question) => {
...
const durationSec = (levelCounts["easy"] || 0) * 20 + (levelCounts["moderate"] || 0) * 35 + (levelCounts["difficult"] || 0) * 60 + (levelCounts["extreme"] || 0) * 90;
      ...
      //   exam?.durationSec ?? questions.length * secondsPerQuestion.easy
      // );
        ...
        <Table stickyHeader>
          ...
          <TableBody>
            {list.map((exam) => {
              ...
              const secPerQ = normalizeSecondsPerQuestion(exam.secondsPerQuestion);
              const totalQ = Number(exam.totalQuestions ?? exam.questionIds?.length ?? 0);
              // recor.durationSec = totalQ * (secPerQ.easy+secPerQ.; // default duration for display
              // const durationText = typeof exam.secondsPerQuestion === "object"
              //   ? `${secPerQ.easy}/${secPerQ.moderate}/${secPerQ.difficult}/${secPerQ.extreme} sec`
              //   : formatDuration(totalQ * secPerQ.easy);
const durationText="30 mins"
              const isSubmitted = submittedByExamId.get(exam._id) === true;
