...
const barData = useMemo(() => {
  ...
  records.forEach((r) => {
    ...
    subjectScoreMap[r.subject].total += r.score;
    subjectScoreMap[r.subject].count += 1;
    subjectScoreMap[r.subject].best = Math.max(
      subjectScoreMap[r.subject].best,
      r.score
    );
  });
...
<div className="rounded-xl p-5 bg-gradient-to-br from-indigo-50 to-white border border-indigo-100">
  ...
  <YAxis />
  <Tooltip />
 <Bar dataKey="avgScore" fill="#4f46e5" name="Average Score" radius={[6, 6, 0, 0]} />
<Bar dataKey="bestScore" fill="#f59e0b" name="Best Score" radius={[6, 6, 0, 0]} />
